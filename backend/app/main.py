...
#     try:
#         # Check port to decide between SSL and STARTTLS
#         if SMTP_PORT == 465:
#             # Use implicit SSL
...
#         # Log the OTP to console as a failsafe so you can still login
#         logger.info(f"FAILSAFE: OTP for {email} is {otp}")
#         return True # Return True to allow the frontend flow to continue
...
#     """Helper to safely get Kp index, handling None values"""
...
#             weather_data = storm_simulator.get_simulated_weather(scenario_enum, latitude, longitude)
#         else:
#             weather_data = await noaa_service.get_current_space_weather()
        
#         scenario = "simulation" if current_simulation["active"] else "normal"
#         processed_data = risk_service.process_space_weather_data(weather_data, latitude, scenario)
...
#             SpaceWeatherData(timestamp=datetime.utcnow(), kp_index=2.0, risk_level="low", estimated_gps_error_m=(5, 15), alerts=["Data fetch failed"], source="FALLBACK"),
#             latitude, "normal"
#         )
...
#         current_simulation.update({"active": True, "scenario": scenario, "latitude": latitude, "longitude": longitude})
#         weather_data = storm_simulator.get_simulated_weather(scenario_enum, latitude, longitude)
#         current_simulation["kp_index"] = weather_data.kp_index
#         await cache.delete("route_cache_*")
        
#         await broadcast_update("simulation_started", {"scenario": scenario, "data": weather_data.dict()})
#         return weather_data
#     except Exception as e:
...
#     current_simulation.update({"active": False, "scenario": None, "kp_index": None})
#     return {"message": "Simulation stopped"}

# @app.get("/api/space-weather/timeline")
# async def get_storm_timeline(scenario: str = "severe"):
#     scenario_enum = SimulationScenario(scenario)
#     return {"timeline": storm_simulator.generate_storm_timeline(scenario_enum), "scenario": scenario}
...
#     return {**result, "simulation_type": "gps_failure", "timestamp": datetime.utcnow().isoformat()}

# @app.get("/api/health", response_model=HealthResponse)
# async def health_check():
#     return HealthResponse(status="healthy", timestamp=datetime.utcnow(), services={"cache": True, "noaa": True, "routing": True, "simulation": True, "heatmap": True})

# @app.get("/api/status")
# async def get_system_status():
#     return {"timestamp": datetime.utcnow().isoformat(), "simulation_active": current_simulation["active"], "services": {"noaa": "operational", "routing": "operational"}}
...
#         try: await connection.send_json({"type": event_type, "data": data})
#         except: pass
